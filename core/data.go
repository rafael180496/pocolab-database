...
import (
	...
	casting "github.com/rafael180496/pocolab-utils/casting"
...
func (p *StSQLData) ToJSON() casting.JSON {
	var data casting.JSON = p.Data
	return data
}

/*ToMap : Convierte un StSQLData a map interface*/
func (p *StSQLData) ToMap() ([]map[string]interface{}, error) {
	return casting.JSONtoObj(p.ToJSON())
}
...
func FindTp(v interface{}, tp TpCore) interface{} {
	switch tp {
	case INTP:
		return casting.ToInt(v)
	case STTP:
		return casting.ToString(v)
	case DTTP:
		vl := casting.ToString(v)
		date, err := casting.StringToDate(vl)
		return utl.ReturnIf(err != nil, utl.FNulo(), date)
	case FLTP:
		return casting.ToFloat(v)
	case BLTP:
		return casting.ToBoolean(v)
	case JSONTP:
		data, _ := casting.NewJSON(v)
		return data
	default:
		return nil
	}
